...
const VERSION = 'v74.1-brtireplica';
...
function validateSnapshot(payload, streamState, now=nowMs()){
  ...
  const settlementGrade = !!(payload?.rawMarket?.summary?.settlementGrade || payload?.brti?.settlementGrade || payload?.brti?.source === 'CF_BRTI');
  if(!settlementGrade && vs.freshVenueCount < 2) errors.push(`not enough fresh venues (${vs.freshVenueCount})`);
  if(settlementGrade && vs.freshVenueCount < 1) errors.push('settlement-grade BRTI venue missing');
  ...
  return { ok: errors.length === 0, errors, warnings, snapshotAgeMs, target, price, minutesLeft, venue: vs, series, digest, settlementGrade: !!settlementGrade };
}
...
function localIndependentRead(payload, validation){
  ...
  const settlementGrade = !!(payload?.rawMarket?.summary?.settlementGrade || payload?.brti?.settlementGrade || payload?.brti?.source === 'CF_BRTI');
  const dataTier = !v.ok ? 'F' : (settlementGrade ? 'A' : (v.venue.freshVenueCount >= 4 && (v.venue.maxSpreadBps == null || v.venue.maxSpreadBps <= 8) && series.length >= 12 ? 'A' : (v.venue.freshVenueCount >= 3 ? 'B' : 'C')));
  let tradeRead = 'WAIT';
...
function publicResponse(read, validation, startedAt=nowMs()){
  ...
  return {
    ...
    validation: { ok: validation.ok, errors: validation.errors, warnings: validation.warnings, freshVenueCount: validation.venue.freshVenueCount, quotedVenueCount: validation.venue.quotedVenueCount, maxSpreadBps: validation.venue.maxSpreadBps, settlementGrade: !!(validation && validation.settlementGrade) },
    debug: read.debug,
    ai: {
      health: read.software_health,
      trade_read: read.trade_read,
