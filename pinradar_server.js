...
const VENUES = {
  coinbase: mkVenue('coinbase', { weight: 1.0 }),
  kraken: mkVenue('kraken', { weight: 0.8 }),
  bitstamp: mkVenue('bitstamp', { weight: 0.5 })
};
const VENUE_LIST = () => Object.values(VENUES);

function bookApply(v, side, price, size) {
  const m = side === 'bid' ? v.book.bids : v.book.asks;
  if (!(size > 0)) m.delete(price); else m.set(price, size);
  if (m.size > 600) {  // guard against unbounded growth on a bad feed
    const keys = [...m.keys()].sort((a, b) => side === 'bid' ? b - a : a - b);
    for (const k of keys.slice(400)) m.delete(k);
  }
}
function bookRecompute(v, now) {
  ...
  if (ba <= bb) return;                        // crossed/locked book: reject the tick
  const mid = (bb + ba) / 2;
  const band = mid * BAND_FRAC;
  let bd = 0, ad = 0;
  for (const [p, s] of v.book.bids) if (mid - p <= band && p <= mid) bd += s;
  for (const [p, s] of v.book.asks) if (p - mid <= band && p >= mid) ad += s;
  ...
  coinbase: {
    ...
    onMsg(v, j, now) {
      ...
      if (j.type === 'snapshot') {
        ...
        for (const [p, s] of (j.bids || [])) bookApply(v, 'bid', Number(p), Number(s));
        for (const [p, s] of (j.asks || [])) bookApply(v, 'ask', Number(p), Number(s));
        bookRecompute(v, now); return true;
      }
      if (j.type === 'l2update') {
        for (const ch of (j.changes || [])) {
          const side = ch[0] === 'buy' ? 'bid' : 'ask';
          bookApply(v, side, Number(ch[1]), Number(ch[2]));
        }
        bookRecompute(v, now); return true;
      }
  ...
  kraken: {
    ...
    onMsg(v, j, now) {
      if (j.channel === 'book' && Array.isArray(j.data)) {
        ...
        for (const b of (d.bids || [])) bookApply(v, 'bid', Number(b.price), Number(b.qty));
        for (const a of (d.asks || [])) bookApply(v, 'ask', Number(a.price), Number(a.qty));
        bookRecompute(v, now);
  ...
  bitstamp: {
    ...
    onMsg(v, j, now) {
      if (j.event === 'data' && j.data && Array.isArray(j.data.bids)) {
        v.book.bids.clear(); v.book.asks.clear();
        for (const [p, s] of j.data.bids) bookApply(v, 'bid', Number(p), Number(s));
        for (const [p, s] of j.data.asks) bookApply(v, 'ask', Number(p), Number(s));
        bookRecompute(v, now);
...
function connectVenue(name) {
  ...
  const onOpen = () => {
    ...
    try { ws.send(JSON.stringify(a.sub)); if (a.sub2) ws.send(JSON.stringify(a.sub2)); } catch (_) { }
    armWatchdog(v);
  };
  const onMessage = (ev) => {
...
function settlementGeometry({ indexNow, strike, tau, sigmaBpsRootSec, driftBpsSec, strikeType }) {
  ...
  /* Clamped to [1%,99%]. Calibration against live Kalshi implied showed a
     mean absolute error near 7 pp; reporting 99.95% would claim precision
     the model has not demonstrated. */
  const pAbove = clamp(normCdf(z), 0.01, 0.99);
  ...
  return {
    ...
    sigmasOfCushion: sigmasOfCushion == null ? null : round(sigmasOfCushion, 3),
...
function fuse({ sg, flow, kal, sent, side, tau, quality }) {
  ...
  const pAdverse = sg ? (mySide === 'ABOVE' ? 1 - sg.pAboveStrike : sg.pAboveStrike) : null;
  ...
  let pinPressure = clamp(Math.round(Math.max(raw, lateFlowFloor, lateSentFloor)), 0, 100);
...
async function radar(payload) {
  ...
  const out = fuse({ sg, flow, kal, sent, side, tau, quality });
    ...
    if (req.method === 'GET' && u.pathname === '/health') {
      ...
      return send(res, 200, {
        ...
        venues: Object.fromEntries(VENUE_LIST().map(v => [v.name, {
          ...
          lastError: v.lastError, trades60s: v.trades.filter(t => t[0] > now - 60000).length
        }])),
