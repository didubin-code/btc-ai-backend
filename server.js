...
const SERVER_VERSION = 'v118-conviction-hold';
...
function enforceStability(st, decision, snapshot) {
  ...
  // Observation period: no directional commands or commitments on immature data (EXIT already passed through above)
  const spanSec = Number(d.dataSpanSec);
  if (Number.isFinite(spanSec) && spanSec < 120) {
    decision.action = 'WAIT';
    decision.enforced = 'OBSERVING';
    decision.why = 'OBSERVING: building baseline (' + Math.round(spanSec) + 's of 120s). No directional commitments on immature data. ' + String(decision.why || '');
    return decision;
  }
