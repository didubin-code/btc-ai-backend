...
const AI_MAX_BODY_CHARS = Number(process.env.AI_MAX_BODY_CHARS || 1800);
const AI_MAX_OUTPUT_TOKENS = Number(process.env.AI_MAX_OUTPUT_TOKENS || 90);
...
async function fetchWithTimeout(url, ms = 4500) {
  ...
  try {
    const r = await fetch(url, { signal: ctl.signal, headers: { 'user-agent': 'btc-engine-v95-execution-lane-fast-safe' } });
...
async function marketSnapshot() {
  ...
  return {
    version: 'v95-execution-lane-fast-safe-server',
    price: proxy,
    proxy,
    brtiProxy: proxy,
    venues: good.length,
...
async function handleCopilot(req, res, mode) {
  ...
  const prompt = [
    'You are a concise BTC execution copilot. Use only the snapshot.',
    'Do not invent prices. Do not give long explanation.',
    'Return ONE short line: ACTION | reason | risk. Prioritize speed, no prose.',
    '',
    snapshot
  ].join('\n');
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/health')) {
      return send(res, 200, {
        ok: true,
        service: 'btc-engine-v95-execution-lane-fast-safe',
...
server.listen(PORT, () => console.log(`btc-engine-v95-execution-lane-fast-safe listening on ${PORT}`));
