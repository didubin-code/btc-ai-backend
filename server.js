...
const SERVER_VERSION = 'v119-independent-safety';
...
function enforceStability(st, decision, snapshot) {
  ...
  if (action === 'EXIT') {
    ...
    const flipRising = Number(d.pFlipSettleDelta60) >= 0.08;
    const localExit = String(d.exitSignal || 'NONE') !== 'NONE';
    const healthy = snapshot && snapshot.activeTrade && Number.isFinite(pWin) && pWin >= 0.55 && !flipRising && !localExit;
    if (!healthy) { st.pendingExit = null; return decision; } // deteriorating or corroborated: exit passes instantly
    const pe = st.pendingExit;
    if (pe && (Date.now() - pe.ts) <= 45000) { st.pendingExit = null; decision.enforced = 'EXIT_CONFIRMED_CONSECUTIVE'; return decision; }
    st.pendingExit = { ts: Date.now() };
    decision.action = 'HOLD_' + snapshot.activeTrade;
    decision.enforced = 'EXIT_PENDING';
    decision.why = 'EXIT urged but position is healthy (win ' + Math.round(pWin * 100) + '%, settle-opposite not rising, local floor silent). Confirm on next read or cite deterioration. ' + String(decision.why || '');
    return decision;
  }
  if (action !== 'EXIT') st.pendingExit = null;
  if (!side) return decision; // WAIT neither advances nor cancels a pending reversal
  // Observation period is now a short soft warmup. Do not miss a 15-minute window merely because the page/backend restarted.
  const spanSec = Number(d.dataSpanSec);
  const la = (snapshot && snapshot.localAdvisory) || {};
  const localCmd = String(la.command || '');
  const localSide = sideOfAction(localCmd);
  const localHighTrade = localCmd.startsWith('TRADE_') && String(la.hardVeto || 'NONE') === 'NONE' &&
    ((Number.isFinite(Number(la.confidence)) && Number(la.confidence) >= 80) ||
     (Number.isFinite(Number(d.pSettleModelSide)) && Number(d.pSettleModelSide) >= 0.80));
  if (Number.isFinite(spanSec) && spanSec < 25 && !localHighTrade) {
    decision.action = 'WAIT';
    decision.enforced = 'OBSERVING_SOFT';
    decision.why = 'OBSERVING: building initial baseline (' + Math.round(spanSec) + 's of 25s soft warmup). High-conviction local/derived entries can bypass this; current read did not. ' + String(decision.why || '');
    return decision;
  }
  // Directional command must cite concrete evidence (schema requires evidence[]); parroting without work is demoted
  const ev = Array.isArray(decision.evidence) ? decision.evidence.filter(x => String(x).trim().length >= 3) : [];
  const hasFlipEvidence = decision.invalidation_hit === true && String(decision.flip_evidence || '').trim().length >= 8;
  if (String(action).startsWith('TRADE') && ev.length < 2 && !hasFlipEvidence) {
    const tradeSide = sideOfAction(action);
    const pModel = Number(d.pSettleModelSide);
    const localCmd2 = String(((snapshot && snapshot.localAdvisory) || {}).command || '');
    const localConf2 = Number(((snapshot && snapshot.localAdvisory) || {}).confidence);
    const localHardVeto2 = String(((snapshot && snapshot.localAdvisory) || {}).hardVeto || 'NONE');
    const localSupports = tradeSide && sideOfAction(localCmd2) === tradeSide && localCmd2.startsWith('TRADE_') && localHardVeto2 === 'NONE' &&
      ((Number.isFinite(pModel) && pModel >= 0.80) || (Number.isFinite(localConf2) && localConf2 >= 80));
    if (localSupports) {
      decision.evidence = [
        'pSettleModelSide ' + (Number.isFinite(pModel) ? pModel.toFixed(2) : 'n/a') + ' supports ' + tradeSide,
        'localAdvisory ' + localCmd2 + ' confidence ' + (Number.isFinite(localConf2) ? Math.round(localConf2) : 'n/a') + '% with hardVeto NONE'
      ];
      decision.enforced = 'EVIDENCE_SYNTHESIZED_FROM_TELEMETRY';
      decision.why = 'Evidence[] was incomplete, but telemetry independently supports ' + tradeSide + '; synthesized evidence instead of false WAIT. ' + String(decision.why || '');
    } else {
      decision.action = 'WAIT';
      decision.enforced = 'EVIDENCE_REQUIRED';
      decision.why = 'Directional call demoted: cite at least two concrete data points in evidence[]. ' + String(decision.why || '');
      return decision;
    }
  }
  if (!st.committedSide) {
    // Earned commitment: strong first mature read (>=72), or two consecutive agreeing reads (>=65) within 60s
    if (conf >= 72) { st.committedSide = side; st.committedConf = conf; st.committedSinceTs = Date.now(); st.pendingRev = null; st.pendingCommit = null; return decision; }
    const pc = st.pendingCommit;
    if (pc && pc.side === side && (Date.now() - pc.ts) <= 60000 && conf >= 65) {
      st.committedSide = side; st.committedConf = conf; st.committedSinceTs = Date.now(); st.pendingRev = null; st.pendingCommit = null; return decision;
    }
    if (conf >= 65) st.pendingCommit = { side, ts: Date.now() };
    return decision;
  }
  st.pendingCommit = null;
...
function applyLocalTradeBackstop(decision, snapshot) {
  const la = (snapshot && snapshot.localAdvisory) || {};
  const d = (snapshot && snapshot.derived) || {};
  const localCmd = String(la.command || '');
  const localSide = sideOfAction(localCmd);
  const localHardVeto = String(la.hardVeto || 'NONE');
  const localTrade = localCmd.startsWith('TRADE_') && localSide && localHardVeto === 'NONE';
  if (!localTrade) return decision;

  const pModel = Number(d.pSettleModelSide);
  const localConf = Number(la.confidence);
  const highConviction = (Number.isFinite(pModel) && pModel >= 0.80) || (Number.isFinite(localConf) && localConf >= 80);
  if (!highConviction) return decision;

  const action = normalizeAction(decision.action);
  const aiSide = sideOfAction(action);
  const aiConf = Number(decision.confidence);
  const aiP = Number(decision.pSettle);
  const aiOppositeStrong = aiSide && aiSide !== localSide && action.startsWith('TRADE_') &&
    Number.isFinite(aiConf) && aiConf >= Math.max(88, (Number.isFinite(localConf) ? localConf + 10 : 88)) &&
    Number.isFinite(aiP) && aiP >= 0.86 && decision.invalidation_hit === true;

  if (action === 'WAIT' || (!aiSide && action !== 'EXIT') || (aiSide && aiSide !== localSide && !aiOppositeStrong)) {
    decision.action = 'TRADE_' + localSide;
    decision.confidence = Math.max(
      Number.isFinite(aiConf) ? aiConf : 0,
      Number.isFinite(localConf) ? localConf : 0,
      Number.isFinite(pModel) ? Math.round(pModel * 100) : 0
    );
    decision.pSettle = Number.isFinite(pModel) ? pModel : (Number.isFinite(aiP) ? aiP : decision.pSettle);
    decision.thesis = localSide;
    decision.timing = 'NOW';
    decision.enforced = 'LOCAL_HIGH_CONVICTION_BACKSTOP';
    decision.evidence = Array.isArray(decision.evidence) && decision.evidence.length >= 2 ? decision.evidence : [
      'localAdvisory ' + localCmd + ' confidence ' + (Number.isFinite(localConf) ? Math.round(localConf) : 'n/a') + '%',
      'pSettleModelSide ' + (Number.isFinite(pModel) ? pModel.toFixed(2) : 'n/a') + ' and hardVeto NONE'
    ];
    decision.why = 'False-WAIT backstop: local high-conviction ' + localCmd + ' with hardVeto NONE cannot be demoted by advisory WAIT/formatting. ' + String(decision.why || '');
  }
  return decision;
}
...
async function callOpenAI(snapshot, st) {
  ...
  const system = `You are an INDEPENDENT SECOND TRADER for 15-minute BTC binary event contracts that settle ABOVE or BELOW a strike. You receive live market data, pSettleAbove, drift, momentum, reversal score, seconds remaining, activeTrade, pEnteredWin, local advisory, prior AI thesis memory, and (when available) kalshi: the real event-market for this exact contract with marketImpliedAbove (crowd probability), yesBid/yesAsk cents, edgeCents (model minus market, positive = model sees value the market does not), and bookImbalance -1..1 (resting order pressure, positive = ABOVE demand). WORK ORDER: form your own read of the market from tapeDigest, derived, and kalshi FIRST; only then compare against localAdvisory, which is merely another trader's rules-based view included for reference — agreeing with it is fine only when the data independently supports it, and your value is precisely the reads it cannot make. EVERY directional command (TRADE_*/HOLD_*) must cite at least two concrete data points with values in evidence[] (e.g. 'gapPathBps -2 to -9 widening', 'kalshiImplied lagging model by 8c'); directional commands should cite evidence[], but incomplete formatting will be repaired from telemetry when the derived/local evidence is already decisive. During the first 25 seconds of data (derived.dataSpanSec < 25), usually WAIT and observe unless derived/local telemetry already shows a high-conviction executable entry (pSettleModelSide >= 0.80 with hardVeto NONE) or EXIT for an open position; do not miss a 15-minute window because of a long warmup. REVERSAL STATS: derived.pTouchStrike = first-passage probability that price crosses the strike at least once before expiry; derived.pFlipSettle = probability of settling on the opposite side. Interpret them together: high pTouchStrike with low pFlipSettle means expect a scare that likely recovers (do not panic-exit a healthy position on the touch). EXIT DISCIPLINE: a price wiggle against a position with pEnteredWin >= 0.55 and flat pFlipSettleDelta60 is expected noise, NOT exit grounds; the system will defer such exits pending confirmation, so cite pEnteredWin or pFlipSettleDelta60 in your why when commanding EXIT. pFlipSettle RISING across reads is the true reversal signal and grounds for exit or flip evidence; derived.pFlipSettleDelta60 and pTouchDelta60 give you those 60-second changes directly (e.g. pFlipSettleDelta60 +0.07 = settle-opposite up 7 points in a minute) and are exactly the kind of citable flip_evidence the system accepts. tapeDigest gives your tape pre-digested: bars15s = net bps change per 15s bar oldest to newest; gapPathBps = distance to strike at -90/-60/-30/now seconds; pAbovePath and kalshiImpliedPath = recent trajectories oldest to newest. REASON ABOUT DIRECTION, NOT JUST LEVEL: gap widening toward a side with pAbovePath trending the same way while the crowd (kalshiImpliedPath) lags is the ahead-of-the-curve entry; evidence decaying from a high level is a warning even when the level still looks strong. Treat market disagreement as evidence: large positive edge with supportive bookImbalance strengthens entry; the market strongly against your thesis with opposing imbalance is a caution or exit factor; near-zero edge means no value at current prices regardless of direction. Priority: (1) EXIT immediately if activeTrade is broken: pEnteredWin under about 0.35 with adverse drift/acceleration; action EXIT. (2) Predict settlement, not current spot. Weight drift, acceleration, and probability trajectory. (3) THESIS DISCIPLINE: prior_ai_state.committed is YOUR OWN standing thesis and prior_ai_state.lastDecisions are YOUR recent reads. First check: if nothing material changed versus your last decision, REPEAT the same action and confidence — do not re-derive from scratch. To REVERSE your committed side you MUST evaluate your previously stated invalidation verbatim; if it triggered, set invalidation_hit=true and put the specific data in flip_evidence (e.g. 'gap crossed strike: distanceTargetBps -3.2' or 'drift flipped to -0.08, pSettleAbove now 0.31'). Reversals without flip_evidence are delayed by the system, so do not bother emitting them. (4) CONFIDENCE = your own settlement probability for your thesis side, as a percent (pSettle x 100). Do not inflate beyond evidence and do not sandbag: if you genuinely read 78% settle-below, say 78. Keep it STABLE between reads unless the data materially moved; never exceed 95. (5) Disagree with local advisory when warranted. FINAL ACTIONS ONLY: WAIT, TRADE_ABOVE, TRADE_BELOW, HOLD_ABOVE, HOLD_BELOW, EXIT. WATCH is forbidden as a final action. If you have a side with confidence >=75 or pSettle for that side >=0.75, return TRADE_ABOVE or TRADE_BELOW, not WAIT. If not executable now, return WAIT. Return valid compact JSON only with exactly these keys: {"action":"WAIT|TRADE_ABOVE|TRADE_BELOW|HOLD_ABOVE|HOLD_BELOW|EXIT","confidence":0-100,"pSettle":0-1,"thesis":"ABOVE|BELOW|NONE","why":"one decisive sentence","risk":"short risk","invalidation":"concrete check","invalidation_hit":false,"timing":"NOW|WAIT","evidence":["p","drift","momentum"]}.`;
...
async function handleAi(req, res) {
  ...
  try {
    ...
    decision = normalizeDecision(decision);
    decision = applyLocalTradeBackstop(decision, snapshot);
    decision = normalizeDecision(decision);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && (u.pathname === '/' || u.pathname === '/health')) {
      return send(res, 200, { ok: true, version: SERVER_VERSION, service: 'btc-ai-backend', openaiEnabled: ENABLE_OPENAI, model: OPENAI_MODEL, deepModel: OPENAI_MODEL_DEEP, dayCalls, cap: AI_MAX_CALLS_PER_DAY, ts: Date.now() });
    }
    ...
    if (req.method === 'GET' && (u.pathname === '/market' || u.pathname === '/btc')) {
      try { const _m = await getMarket(); return send(res, 200, { ..._m, version: SERVER_VERSION }); }
...
try { module.exports = { parseAiContent, normalizeDecision, normalizeAction, enforceCommitment, enforceStability, applyLocalTradeBackstop, chooseModel, fetchJson, getMarket }; } catch (_) {}
